...
### solve and plot examples
function solve_sequential(nm; do_plot=true)
	grid = ExtendableSparseParallel.getgrid(nm; x0=-1.0, x1=1.0)
	n    = num_nodes(grid)
	mat0 = ExtendableSparseMatrix{Float64, Int64}(n, n)
	rhs  = zeros(n)
	t1   = assemble!(mat0,rhs,δ,f,α,β,grid)
	
	sol = SparseArrays.SparseMatrixCSC(mat0) \ rhs
	if do_plot
		scalarplot(grid, sol, Plotter=PyPlot)
	end		
	sol
end
...
function solve_parallel(nm, nt; depth=2, do_plot=true)
	...
	rhs  = zeros(n)
	t5   = assemble_part_para_ESMP_essential3!(mat2,rhs,δ,f,α,β,grid)
	
	
	ExtendableSparseParallel.flush!(mat2)
	sol = mat2.cscmatrix \ rhs
	sol = reorder(sol, mat2.rev_new_indices)
